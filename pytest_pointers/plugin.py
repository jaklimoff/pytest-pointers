...
def _pointer_marker(request):
    if not request.config.option.pointers_collect and not request.config.option.pointers_report:
        return
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: Union[int, pytest.ExitCode]):
